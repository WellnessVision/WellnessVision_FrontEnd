...
const NU_ViewPreviousPhysicalEvents: React.FC = () => {
  ...
  const fetchEvents =  useCallback(async () => {
    try {
      const response = await axios.get<PhysicalEvent[]>(`http://localhost:15000/getUpcomingPhysicalEventsForUsers`, {
        params: { eventState: "Previous", searchCode}
      });
  ...
  const handleViewDetails = (eventId: number, hpId: number) => {
    navigate(`/NU_ViewOnePreviousPhysicalEvents/${eventId}/${hpId}`);
  };
...
export default NU_ViewPreviousPhysicalEvents;
