...
interface NU_DeletePhysicalEventBookingProps {
  show: boolean;
  handleClose: () => void;
  AppointmentBookingsDetails: any;
...
const NU_DeleteAppointmentBookingFineDetails: React.FC<NU_DeletePhysicalEventBookingProps> = ({ show, handleClose, AppointmentBookingsDetails, setDeleteStateFun}) => {
    ...
    const [bookingId, setBookingId] = useState(AppointmentBookingsDetails.bookingId);
  

    const fetchFineData = async () => {
      setFineAmountDetails(null);
        try {
          const response = await axios.get<fineAmountData>(`http://localhost:15000/getAppointmentBookingFineAmountForNU`, {
            params: { bookingId }
          });
...
const eventBookingDeletion = useCallback(async (fineAmount: number, depositAmount: number, twoDaysBeforeState: string) => {
    try {
      ...
      await axios.put(`http://localhost:15000/deleteAppointmentBookingForNU`, null, {
        params: {
            userId,
            bookingId,
            fineAmount,
      ...
      navigate('/NU_ViewAllBookedUpcomingAppointments');
  ...
  }, [userId, bookingId, navigate, setError]);
    ...
    export default NU_DeleteAppointmentBookingFineDetails;