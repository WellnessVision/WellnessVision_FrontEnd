...
const NU_ViewBookedPreviousphysicalEvents: React.FC = () => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.get<PhysicalEvent[]>(`http://localhost:15000/getBookedUpcomingPhysicalEventsForUsers`, {
        params: { userId, bookingState: "Booking", eventState: "Previous" }
      });
  ...
  return (
    ...
    <NU_Sidebar activeMenuItem={["PreviousPhysicalBooked_Events", "PreviousBooked_Events", "Booked_Events"]}/>
      <div className={`blurBackground ${showPopup ? 'blur' : ''}`}>
        <h3 className="header">Booked Previous Events</h3>
...
export default NU_ViewBookedPreviousphysicalEvents;
