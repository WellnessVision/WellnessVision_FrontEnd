...
const Volunteer_ViewOneNotification: React.FC<ViewOneNotificationProps> = ({ show, handleClose, NotificationId}) => {
...
const handleMarkAsParticipate = useCallback(async (notificationId: number, readState: string) => {
    try {
      ...
      navigate(`/Volunteer_Notification`);
    } catch (err) {
      if (err instanceof Error) {
        setError(err.message);
      } else {
    ...
    export default Volunteer_ViewOneNotification;