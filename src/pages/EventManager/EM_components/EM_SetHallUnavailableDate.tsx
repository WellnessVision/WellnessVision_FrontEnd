...
const EM_SetHallUnavailableDate: React.FC<HP_EventBookingCloseProps> = ({ show_5, handleClose_5, hallId, setSelectUnavailableDate }) => {
  ...
  const checkHallAvailability = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
        const response = await axios.get<Event>(`http://localhost:15000/checkAndSetUnavailableDateOfHallForEventManager`, {
            params: { 
                hallId, 
                unavailableDate
            }
...
return (
    <div className={`popup ${show_5 ? 'show HP_SelectOneHallForPhysicalEventPopupLayer' : ''}`}>
      ...
      {unavailableDatesUpdateFlag && <EM_UnavailableDateUpdateState show_6={showPopup_6} handleClose_6={togglePopup_6} eventData={eventData} hallId={hallId}/>}
      {showLoadingPopup && <img className="loading eventBookingCloseForHPLoadingGif" src={loading_gif} alt="Loading..." />}
    </div>
  );
}

export default EM_SetHallUnavailableDate;