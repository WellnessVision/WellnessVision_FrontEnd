...
const EM_SetChargeAndAdvancePrecentage: React.FC<HP_EventBookingCloseProps> = ({ show_7, handleClose_7, hallId, setChargeAndAdvancePrecentage, roomDetails }) => {
  ...
  const [advancePercentage, setAdvancePercentage] = useState(roomDetails.advance_percentage);
  ...
  const checkHallAvailability = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        await axios.put(`http://localhost:15000/updateHallChargeAndAdvancePercentageOfHallForEventManager`, null, {
            params: { 
                hallId, 
                charge, 
                advancePercentage
...
export default EM_SetChargeAndAdvancePrecentage;