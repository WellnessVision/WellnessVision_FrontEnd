...
import AM_SetStartDateAndEndDate from './AM_SetStartDateAndEndDate';
import AM_SetRoomUnavailableDate from './AM_SetRoomUnavailableDate';
...
const AM_SelectMaintainDateType: React.FC<Hp_DeletePhysicalEventFineDetailsProps> = ({ show_2, handleClose_2, roomId, setSelectMaintainDateType, roomDetails}) => {
...
const clearSetDays =  useCallback(async () => {
    try {
        ...
        await axios.put(`http://localhost:15000/clearRoomSetDateOfRoomsForAppointmentManager`, null, {
        params: { 
            roomId 
            }
        });
...
return (
        <div className={`popup ${show_2 ? 'show popup_Hp_DeletePhysicalEventFineDetails' : ''} popup_HP_HallAvailability_popup`} style={{ zIndex: '0' }}>
          ...
          {selectStartAndEndDate && <AM_SetStartDateAndEndDate show_3={showPopup_3} handleClose_3={togglePopup_3} roomId={roomId} setSelectStartAndEndDate={setSelectStartAndEndDate}/>}
          {selectUnavailableDate && <AM_SetRoomUnavailableDate show_5={showPopup_5} handleClose_5={togglePopup_5} roomId={roomId} setSelectUnavailableDate={setSelectUnavailableDate}/>}
        </div>
      );
    };
    
    export default AM_SelectMaintainDateType;