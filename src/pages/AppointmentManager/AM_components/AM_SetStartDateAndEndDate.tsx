...
const AM_SetStartDateAndEndDate: React.FC<HP_EventBookingCloseProps> = ({ show_3, handleClose_3, roomId, setSelectStartAndEndDate }) => {
  ...
  const checkHallAvailability = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
        const response = await axios.get<Event>(`http://localhost:15000/checkAndSetMaintenanceDateOfRoomsForAppointmentManager`, {
            params: { 
                roomId, 
                startDate, 
                endDate
...
return (
    <div className={`popup ${show_3 ? 'show HP_SelectOneHallForPhysicalEventPopupLayer' : ''}`}>
      ...
      {datesUpdateFlag && <AM_MaintainDateUpdateState show_4={showPopup_4} handleClose_4={togglePopup_4} eventData={eventData} roomId={roomId}/>}
      {showLoadingPopup && <img className="loading eventBookingCloseForHPLoadingGif" src={loading_gif} alt="Loading..." />}
    </div>
  );
}

export default AM_SetStartDateAndEndDate;