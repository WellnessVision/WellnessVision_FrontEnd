...
import AM_NotificationIcon from "../../../components/AM_NotificationIcon";
...
const AM_Sidebar: React.FC<NU_SidebarProps> = ({ activeMenuItem }) => {
  const [notificationCount, setNotificationCount] = useState<number>(0);
  const [error, setError] = useState("");
  const [userDetails, setUserDetails] = useState<UserDetailsProps | null>(null);
  const appointmentManagerId = Number(localStorage.getItem('appointmentManagerId'));

  const fetchUserDetails = async () => {
    try {
      const response = await axios.get<UserDetailsProps>(
        `http://localhost:15000/getAppointmentManagerDetailsForAppointmentManager`,
        {
          params: { appointmentManagerId },
        }
      );
    ...
    const fetchEvent = async () => {
      try {
        const response = await axios.get<number>(`http://localhost:15000/getNotificationsCountForAnyUser`,{
            params: { ownerId: appointmentManagerId }
        });
      ...
      >
        <div className="container-md" id="navbar_2">
          ...
          <div className="ml-auto">
            <AM_NotificationIcon count={notificationCount} />
          </div>
          <div className="name">
        ...
        >
          <div className="mt-2">
            ...
            <ul className="nav nav-pills flex-column mt-2 mt-sm-0" id="parentM">
              ...
              >
                <a
                  href="/AM_ViewAllHealthProfessionals"
                  className={`nav-link text-white text-center text-sm-start ${isActive(
                    "HealthProfessionals"
                  )}`}
                  aria-current="page"
              ...
              >
                <a
                  href="/AM_ViewAllAppointmentRooms"
                  className={`nav-link text-white text-center text-sm-start ${isActive(
                    "AppointmentRooms"
                  )}`}
                  aria-current="page"
                >
                  ...
                  <span className="ms-2 d-none d-sm-inline">Appointment Rooms</span>
                </a>
              </li>
              <li>
                <a
