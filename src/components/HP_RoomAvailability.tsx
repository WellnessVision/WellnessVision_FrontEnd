...
import HP_AddAppointmentScheduleOtherDetails from './HP_AddAppointmentScheduleOtherDetails';
...
const HP_RoomAvailability: React.FC<HP_HallAvailabilityProps> = ({ show_3, handleClose_3, eventData, finalDuration }) => {
  ...
  const handleCancelPhysicalEvent = async (event: FormEvent) => {
    ...
    if (event_id) {
      try {
        await axios.put('http://localhost:15000/deleteTemporarilyBookedRoomForAppointmentForHp', {
            event_id
        });
        handleClose_3();
      } catch (error) {
  ...
  return (
    <div className={`popup ${show_3 ? 'show HP_HallAvailabilityPopupForIt' : ''} `}>
      <div className="popup-inner popup-inner_HP_HallAvailability">
        <div className="card HP_HallAvailability_fontSize" style={{ width: '100%' }}>
          <div className="card-body HP_HallAvailabilityMainDivCard">
            <h5 className="card-title title_HP_HallAvailability">Room Availability (In WellnessVision Premises)</h5>
            <div className="straight-line"></div>
            <div className='HP_HallAvailability_div'>
              {eventData ? (
                <div>
                  <p className="card-text detail Hall_ID_HP_HallAvailability"><i className="bi bi-award-fill"></i> {eventData.hall_id} (Room ID)</p>
                  {eventData.hall_type === 'Discussion' && (
                    <p className="card-text detail Hall_type_HP_HallAvailability">
                      <i className="bi bi-bookmark-star-fill"></i> {eventData.hall_type} Room (Table and Chairs)
                    </p>
                  )}
                  {eventData.hall_type === 'Therapy' && (
                    <p className="card-text detail Hall_type_HP_HallAvailability">
                      <i className="bi bi-bookmark-star-fill"></i> {eventData.hall_type} Room (Therapy Beds)
                    </p>
                  )}
                  {eventData.hall_type === 'Free Space' && (
                    <p className="card-text detail Hall_type_HP_HallAvailability">
                      <i className="bi bi-bookmark-star-fill"></i> {eventData.hall_type} Room (Yoga Mats)
                    </p>
                  )}
      ...
      {showPopup_4 && 
                        <HP_AddAppointmentScheduleOtherDetails 
                        show_4={showPopup_4} 
...
export default HP_RoomAvailability;